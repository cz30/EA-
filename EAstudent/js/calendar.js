...
var calendarWidget = {
	 ...
	 * @param {function} monthPreclick	顶头的日期点击上个月
	 * * @param {function} monthNextclick	顶头的日期点击下个月
	...
	initCalculate: function(data) {  
		calendarWidget.config = data || {
			...
			monthPreclick:undefined,//顶头的上一个月点击回调,可以通过此回调查看当前月
			monthNextclick:undefined,//顶头的下一个月点击回调,可以通过此回调查看当前月
	...
	renderCalendar: function() {  //生成日历
		...
		var textYear = calendarWidget.year + "年";
		var textStr = calendarWidget.month + ' 月';  //顶部显示的日期
		var sum = 0;
		var htmlStr = "";
		//获取上一个月的天数
		var TempMonth;
		...
		for (var i = 0; i < firstDayWeek; i++) {
			sum++;
			lastMonthStartDay++;
			htmlStr += '<div class="calendar-col aligncanter calendar-col-lastMonth"><div class="dayValue">' + lastMonthStartDay + '</div></div>';
		}
			...
			for (var z = 0; z < calendarWidget.config.mark.length; z++) { //判断是否为标记点
				if (tempDay == calendarWidget.config.mark[z].time.trim()) {
					...
					if (calendarWidget.config.mark[z].markPoint != false) // 是否显示点
						// textTemp += calendarWidget.config.mark[z].pointText? '<span class="calendar-point">'+calendarWidget.config.mark[z].pointText+'</span>' : '<span class="calendar-point-dot"></span>';
						textTemp += '<span class="calendar-point-dot"></span>';
		...
		if (sum < 42) { // 创建后面的空白，补够五行
			...
			for (var i = sum; i < 42; i++) {
				sum++;
				j++;
				htmlStr += '<div class="calendar-col aligncanter calendar-col-nextMonth"><div class="dayValue">' + j + '</div></div>';
			}
		}
		
		var content = "<div class='calendar-Time-header'><span class='calendar-year'>"+textYear+"</span><div class='calendar-month'><a class='calendar-lastMonth'></a><a class='calendar-TimeH'>" +
			textStr + "<a><a class='calendar-nextMonth'></a></div></div>" +
			'<div class="calendar-content">' +
			'<div class="calendar-row calendar-header">' +
			'<div class="calendar-col aligncanter"><div class="dayValue">日</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">一</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">二</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">三</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">四</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">五</div></div>' +
			'<div class="calendar-col aligncanter"><div class="dayValue">六</div></div>' +
			'</div>' +
	...
	bindEvent: function() { //绑定事件
		...
		$(".calendar-TimeH").on('click',function(){ 
			if(calendarWidget.config.dateclick){
				var textYear = calendarWidget.year;
				var textStr = calendarWidget.month
		...
		$(".calendar-nextMonth").on('click', function() {
			...
			setTimeout(function(){
				...
				if(calendarWidget.config.monthNextclick){
					var textYear = calendarWidget.year;
					var textStr = calendarWidget.month;
					calendarWidget.config.monthNextclick(textYear,textStr);
				}
			},300);
		})
		//头部上月按钮
		$(".calendar-lastMonth").on('click', function() {
			...
			setTimeout(function(){
				...
				if(calendarWidget.config.monthPreclick){
					var textYear = calendarWidget.year;
					var textStr = calendarWidget.month;
					calendarWidget.config.monthPreclick(textYear,textStr);
				}
			},300);
		})
